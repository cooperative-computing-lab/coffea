...
try:
    ...
    from taskvine import Manager, Task
except ImportError:
    vine = None
    print("taskvine module not available")

    class Task:
        def __init__(self, *args, **kwargs):
            raise ImportError("taskvine not available")

    class Manager:
        def __init__(self, *args, **kwargs):
            raise ImportError("tasvine not available")
...
class CoffeaTaskvine(Manager):
    def __init__(
        self,
        executor,
    ):
        self._staging_dir_obj = TemporaryDirectory("taskvine-tmp-", dir=executor.filepath)
    ...
    def _check_executor_parameters(self, executor):
        if executor.environment_file and not executor.wrapper:
            raise ValueError(
                "TaskvineExecutor: Could not find poncho_package_run. Use 'wrapper' argument."
            )
        ...
        # tasvine always needs serializaiton to files, thus compression is always on
    ...
    def _submit_processing_task(self, infile_procc_fn, item):
        ...
        t = ProcCoffeaTaskvineTask(self, infile_procc_fn, item)
        self.submit(t)
    ...
    def _declare_resources(self):
        ...
        mode = vine.VINE_ALLOCATION_MODE_MAX
        if executor.resources_mode == "fixed":
            mode = vine.VINE_ALLOCATION_MODE_FIXED
        ...
        if executor.resources_mode == "max-throughput":
            self.set_category_mode(
                "processing", vine.VINE_ALLOCATION_MODE_MAX_THROUGHPUT
            )
        ...
        for category in "default preprocessing processing accumulating".split():
            if fast_terminate and fast_terminate > 1:
                self.enable_disconnect_slow_workers_category(category, fast_terminate)
    ...
    def _write_fn_wrapper(self):
        ...
        The wrapper created is created/deleted according to the lifetime of the TaskvineExecutor."""
...
class CoffeaTaskvineTask(Task):
    ...
    def remote_command(self, env_file=None):
        ...
        if env_file:
            wrap = (
                './py_wrapper -d -e env_file -u "$VINE_SANDBOX"/{}-env-{} -- {}'
            )
    ...
    def successful(self):
        return (self.result == 0) and (self.exit_code == 0)

    def exhausted(self):
        return self.result == vine.VINE_RESULT_RESOURCE_EXHAUSTION
...
class PreProcCoffeaTaskvineTask(CoffeaTaskvineTask):
    def __init__(self, queue, infile_fn, item, itemid=None):
        if not itemid:
            itemid = "pre_{}".format(CoffeaTaskvineTask.tasks_counter)
    ...
    def clone(self, queue):
        return PreProcCoffeaTaskvineTask(
            queue,
            self.infile_fn,
            self.item,
            self.itemid,
...
class ProcCoffeaTaskvineTask(CoffeaTaskvineTask):
    def __init__(self, queue, infile_fn, item, itemid=None):
        self.size = len(item)

        if not itemid:
            itemid = "p_{}".format(CoffeaTaskvineTask.tasks_counter)
...
class AccumCoffeaTaskvineTask(CoffeaTaskvineTask):
    def __init__(
        self,
        queue,
        infile_fn,
    ...
    def clone(self, queue):
        return AccumCoffeaTaskvineTask(
            queue,
            self.infile_fn,
            self.tasks_to_accumulate,
            self.itemid,
    ...
    def debug_info(self):
        ...
        results = [
            CoffeaTaskvineTask.debug_info(t)
            for t in tasks
            if isinstance(t, AccumCoffeaTaskvineTask)
        ]
        results += [
            t.debug_info() for t in tasks if not isinstance(t, AccumCoffeaTaskvineTask)
        ]
...
def run(executor, items, function, accumulator):
    ...
    if not vine:
        ...
        import taskvine  # noqa

    global _vine_queue
    if _vine_queue is None:
        _vine_queue = CoffeaTaskvine(executor)
    else:
        # if queue already listening on port, update the parameters given by
        # the executor
        _vine_queue.executor = executor
    ...
    try:
        ...
        if executor.desc == "Preprocessing":
            ...
            result = _vine_queue._processing(items, function, accumulator)
            _vine_queue = None
    except Exception as e:
        _vine_queue = None
        raise e
...
def _handle_early_terminate(signum, frame, raise_on_repeat=True):
    ...
    if early_terminate and raise_on_repeat:
        ...
        _vine_queue.console.printf(
            "********************************************************************************"
        )
        _vine_queue.console.printf("Canceling processing tasks for final accumulation.")
        _vine_queue.console.printf("C-c now to immediately terminate.")
        _vine_queue.console.printf(
            "********************************************************************************"
        )
        early_terminate = True
        _vine_queue.cancel_by_category("processing")
...
# Functions related to dynamic chunksize, independent of Taskvine
